...
class AttackTest : AnnotationSpec() {
  ...
  @Test
  fun `useDebuff increases the chosen stat by 0,3 and reduces it by the outcoming value`() { // DebuffAttack Test
    val debuff = DebuffAttack("Attack Debuff", AttackKinds.DEBUFF, Type.NORMAL, 100, 0, 3)
    val exampleStat = TestData.battleStatsHigherAttack.getAttack()

    debuff.useDebuff(exampleStat)
    assertThat(exampleStat).isEqualTo(120)
  }
}